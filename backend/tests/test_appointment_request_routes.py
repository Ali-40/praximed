...
Updated: Sprint 7 / Module 64 — JWT current_user auth wired; header auth tests replaced.
...
JWT_SECRET = "test-secret-appointment-routes-m64"
...
# ---------------------------------------------------------------------------
# Auth enforcement tests (Module 64 — JWT)
# ---------------------------------------------------------------------------
...
def test_wrong_clinic_returns_403(client_wrong_clinic):
    with patch(f"{REPO}.list_appointment_requests", new=AsyncMock(return_value=[])):
...
def test_valid_jwt_can_create_appointment_request(client):
    with patch(f"{REPO}.create_appointment_request", new=AsyncMock(return_value=FAKE_ROW)):
...
def test_valid_jwt_can_archive_appointment_request(client):
    with patch(f"{REPO}.archive_appointment_request", new=AsyncMock(return_value=FAKE_ROW)):
